document.addEventListener("DOMContentLoaded", async function () {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      // Biometric Number validation: optional, but if provided must be exactly 8 digits
      if (biometricNumber && !/^\d{8}$/.test(biometricNumber)) {
        return Swal.fire(
          "Invalid Biometric Number",
          "Biometric number must be exactly 8 digits, or left blank.",
          "warning",
        );
      }
